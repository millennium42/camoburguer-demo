export function mapPostgresError(error, logger) {
  ...
  let isPublic = false;
  let codeStr = undefined;

  // Domain/Client errors that were manually thrown or validated
  if (error.validation) {
    statusCode = 400;
    isPublic = true;
  } else if (!error.code && /inválid|obrigatóri|booleano|deve ter|transição|item|preço|valor/i.test(error.message)) {
    statusCode = 400;
    isPublic = true;
  } else if (error.code) {
    // PG errors mapping
  ...
  } else if (error.code && ['22007', '22008', '22P02', '23505', '23503', '23514'].includes(error.code)) {
