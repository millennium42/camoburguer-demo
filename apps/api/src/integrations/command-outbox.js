...
async function finishUnknown(db, command, workerId, error) {
  ...
  return db.transaction((client) => updateOwnedChannelCommand(command.id, workerId, {
...
async function reconcileCommand({ db, adapter, command, workerId }) {
  ...
  if (result?.state !== "applied") {
    return finishUnknown(db, command, workerId, new Error(result?.reason || "Resultado externo inconclusivo"));
  }
...
async function sendCommand({ db, adapter, command, workerId }) {
  try {
    ...
    return db.transaction((client) => updateOwnedChannelCommand(command.id, workerId, {
      status: classification === "retryable_not_sent" && command.attempts < MAX_ATTEMPTS
        ? "pending"
        : "failed",
      error: sanitizedError(error),
      lastHttpStatus: Number(error?.statusCode || 0) || null,
      nextAttemptAt: new Date(Date.now() + backoffMs(command.attempts)).toISOString(),
      completedAt: classification === "terminal" || command.attempts >= MAX_ATTEMPTS
...
export async function processChannelCommands({
  ...
  for (let processed = 0; processed < limit; processed += 1) {
    const claimed = await db.transaction((client) => claimChannelCommand({
    ...
    results.push(claimed.mode === "send"
      ? await sendCommand(context)
      : await reconcileCommand(context));
  }
  return results;
}
