...
import { assertSafeSimulationBaseUrl, runSimulation } from "../scripts/demo-simulator-client.mjs";
...
function happyApi() {
  ...
  const fetchImpl = async (url, options = {}) => {
    ...
    if (parsed.pathname === "/auth/login") {
      return json({ csrfToken: "csrf-test" }, 200, {
        "set-cookie": "camoburguer_session=test; HttpOnly",
      });
    }
    ...
    if (parsed.pathname === "/cash-shifts")
      return json({ items: [{ id: "shift-1", status: "open" }] });
...
test("simulador conclui somente depois de verificar efeitos e nunca usa id ausente", async () => {
  ...
  assert.equal(
    Object.values(summary.steps).every((step) => step.status === "completed"),
    true,
  );
  assert.equal(
    api.requests.some((request) => /\/(?:undefined|null)(?:\/|$)/.test(request.path)),
    false,
  );
...
test("falha HTTP interrompe dependentes e produz resumo verdadeiro", async () => {
  ...
  assert.equal(
    api.requests.some((request) => request.path === "/orders"),
    false,
  );
});
...
test("guard recusa produção e protocolos não HTTP antes da request", () => {
  ...
  assert.equal(
    assertSafeSimulationBaseUrl("http://host.docker.internal:3001"),
...
test("4xx falha honestamente e entrypoint devolve exit code não zero", async () => {
  ...
  await assert.rejects(
    runSimulation({
      ...
      password: "secret",
      fetchImpl: async (url, options) =>
        new URL(url).pathname === "/inventory"
