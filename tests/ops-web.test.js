...
test("Docker, scripts, pacote API e Render nao dependem mais do static site React", async () => {
  ...
  assert.doesNotMatch(
    dockerfile,
    /ops_web_builder|ops-web\/dist|COPY apps\/ops-web\/package\.json|COPY apps\/ops-web \.\//,
  );
  ...
  assert.doesNotMatch(
    renderYaml,
    /camoburguer-ops-web|VITE_PUBLIC_BASE|VITE_API_BASE|VITE_DEMO_MODE|AUTH_COOKIE_SAMESITE|DEMO_DIRECT_ACCESS/,
  );
