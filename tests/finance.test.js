...
test("timezone de negócio torna buckets e filtros independentes do TZ do processo", () => {
  ...
  assert.equal(filterEntries(entries, {
    dateFrom: "2025-12-31",
    dateTo: "2025-12-31"
  }, { timeZone: "America/Sao_Paulo" }).length, 1);
});

test("cancelamento reduz o método original e reconcilia com o líquido", () => {
  const entries = [
    {
      type: "sale", amount: 100, paymentMethod: "credit_card", source: "counter",
      orderId: "order-cancelled", occurredAt: "2026-07-16T20:00:00.000Z"
    },
    {
      type: "cancellation", amount: -40, paymentMethod: "credit_card", source: "counter",
      orderId: "order-cancelled", occurredAt: "2026-07-16T21:00:00.000Z"
    }
  ];
...
test("processos filhos sob TZ distintos produzem relatório byte a byte idêntico", () => {
  ...
  `;
  const run = (tz) => spawnSync(process.execPath, ["--input-type=module", "-e", script], {
    cwd: process.cwd(),
    encoding: "utf8",
