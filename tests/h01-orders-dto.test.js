...
import { orderFingerprintPayload, fingerprint, canonicalJson } from "../apps/api/src/idempotency.js";
...
test("DTO de pedido avulso repudia propriedades estruturais isoladas", () => {
  for (const field of ["tabId", "roundNumber", "roundKind", "reversesOrderId"]) {
    assert.throws(() => normalizeStandaloneOrderDto({ items: [], [field]: "infiltrado" }), (err) => {
...
test("DTO de pedido avulso repudia combinacao de propriedades estruturais", () => {
  assert.throws(() => normalizeStandaloneOrderDto({
    items: [],
    tabId: "tab-1",
    roundNumber: 2,
...
test("Fingerprint criptografico consolida roundNumber e roundKind para alinhamento com DTO e banco", () => {
  ...
  const payloadRodada = orderFingerprintPayload({ items: [{ sku: "x", quantity: 1 }], tabId: "t1", roundKind: "production" }, { roundNumber: 2 });
  ...
  assert.notEqual(
    ...
    fingerprint(orderFingerprintPayload({ items: [{ sku: "x", quantity: 1 }], tabId: "t1", roundKind: "production" }, { roundNumber: 3 }))
  );
});
...
if (connectionString) {
  test("h01 integracao: POST /orders bloqueia campos estruturais, mantendo estoque intacto e rotas avulsas/dedicadas perenes", async (t) => {
    ...
    t.before(async () => {
      ...
      db = createDb(pool);
      await pool.query("TRUNCATE TABLE users, orders, stock_movements, idempotency_records, tab_payments CASCADE");
      
      const pass = await import("../apps/api/src/auth.js").then(m => m.hashPassword("test1234"));
      const user = await pool.query(
        "INSERT INTO users (id, name, role, login, password_hash) VALUES ($1, $2, $3, $4, $5) RETURNING id",
        [randomUUID(), "Admin", "admin", "admin-h01", pass]
      );
      
      const dir = await mkdtemp(join(tmpdir(), "camoburguer-h01-"));
      target = spawn("node", ["apps/api/src/server.js"], {
        env: { ...process.env, PORT: String(port), DATABASE_URL: connectionString, DATA_DIRECTORY: dir }
      });
      ...
      const loginRes = await fetch(`http://127.0.0.1:${port}/login`, {
        ...
        body: JSON.stringify({ login: "admin-h01", password: "test1234" })
      });
      const cookie = loginRes.headers.get("set-cookie")?.split(";")[0];
      const { csrfToken } = await loginRes.json();
      authHeader = { "Cookie": cookie, "x-csrf-token": csrfToken, "Content-Type": "application/json" };
    });
    ...
    await t.test("Rejeita payload com tabId (400) e nao afeta o banco", async () => {
      ...
      const idem = await pool.query("SELECT * FROM idempotency_records WHERE idempotency_key = $1", [idempKey]);
    ...
    await t.test("Rejeita payload com roundKind (400)", async () => {
      const res = await fetch(`http://127.0.0.1:${port}/orders`, {
        ...
        body: JSON.stringify({ items: [{ sku: "x-simples", quantity: 1 }], roundKind: "cancellation" })
      });
      assert.equal(res.status, 400);
    ...
    await t.test("Rejeita payload com propriedade desconhecida (400)", async () => {
      const res = await fetch(`http://127.0.0.1:${port}/orders`, {
        method: "POST",
        headers: { ...authHeader, "Idempotency-Key": `h01-${randomUUID()}` },
        body: JSON.stringify({ items: [{ sku: "x-simples", quantity: 1 }], forceAdminOverride: true })
      });
      assert.equal(res.status, 400);
      assert.equal((await res.json()).code, "UNKNOWN_FIELD");
    });
    
    await t.test("Aceita DTO avulso e aplica idempotencia validada com fingerprint exato", async () => {
      const idempKey = `h01-legitimo-${randomUUID()}`;
      const payload = { items: [{ sku: "x-simples", quantity: 1 }], source: "counter", customerName: "Joao" };
