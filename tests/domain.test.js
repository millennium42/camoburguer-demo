...
test("catálogo reflete o snapshot OlaClick de 2026-07-16", () => {
  ...
  assert.deepEqual(
    ...
    { sku: "01-camobuger", name: "01 CAMOBUGER + BATATA FRITA", category: "Lanches", price: 35, description: "", stockCategory: "hamburguer", allowsAddons: true, preparationMode: "kitchen", available: true }
  );
  assert.equal(CATALOG.find((item) => item.sku === "produto-19").available, false);
  assert.equal(createHash("sha256").update(JSON.stringify(CATALOG)).digest("hex"), "295952c490b1fa91c6a24c2b6ba55801d03740b5d2d343f250d7d555d568ea9e");
});
...
test("estoque usa a classificação congelada na linha e preserva fallback legado", () => {
  ...
  assert.deepEqual(calculateStockRequirements([
    { sku: "x-simples", stockCategory: null, quantity: 3 }
  ]), {});
});
...
test("pedido manual rejeita SKU fora do catálogo operacional", () => {
  ...
  }, { allowCustomItems: true }), /Modo de preparo/);
  assert.throws(() => createOrder({
    items: [{ name: "Livre", quantity: 1, price: 10, stockCategory: "bebida" }]
  }, { allowCustomItems: true }), /Categoria de estoque/);
  assert.throws(() => createOrder({
...
test("ticket separa preparo e entrega direta e pedido somente direto nasce pronto", () => {
  const mixed = createOrder({ items: [
    { sku: "x-simples", quantity: 1 },
    { sku: "refrigerante-lata", quantity: 1 }
  ] });
  assert.equal(requiresKitchenPreparation(mixed.items), true);
  assert.match(buildKitchenTicket(mixed), /PREPARO COZINHA[\s\S]*X-SIMPLES[\s\S]*ENTREGA DIRETA — NÃO PREPARAR[\s\S]*Refrigerante lata/);
...
test("pedido aplica desconto por item antes do desconto geral e valida os limites", () => {
  ...
  assert.equal(createOrder({ discountPercent: 100, items: [{ name: "Cortesia", price: 10 }] }, { allowCustomItems: true }).total, 0);
  assert.throws(
    () => createOrder({ discountPercent: 100.01, items: [{ name: "Burger", price: 10 }] }, { allowCustomItems: true }),
    /Desconto do pedido/
  );
  assert.throws(
    () => createOrder({ items: [{ name: "Burger", price: 10, discountPercent: -0.01 }] }, { allowCustomItems: true }),
    /Desconto do item/
  );
});
...
test("delivery exige endereço e preserva o horário informado", () => {
  ...
  assert.throws(
    () => createOrder({ fulfillmentMode: "delivery", items: [{ name: "Burger", price: 30 }] }, { allowCustomItems: true }),
    /Endereço/
  );
  const order = createOrder({
