...
async function requestAs(server, session, path, {
  method = "GET",
  body,
...
async function readSseData(stream, timeoutMs = 3000) {
  ...
  const line = decoder.decode(result.value).split("\n").find((item) => item.startsWith("data: "));
  return line
    ? { done: false, data: JSON.parse(line.slice(6)) }
...
const persistedFixtures = {
  ...
  idempotency_records: (client) => client.query(`
...
if (!connectionString) {
  ...
  describe("seed demo em PostgreSQL efêmero validado", { concurrency: false }, () => {
    ...
    test("HTTP real aplica default-deny, CSRF por sessao e matriz RBAC", async () => {
      ...
      await pool.query(
        `INSERT INTO users (id, username, role, password_hash)
         VALUES ('operator-test', 'operator-test', 'operator', $1),
                ('kitchen-test', 'kitchen-test', 'kitchen', $1)
      ...
      try {
        ...
        for (const [path, method] of protectedFamilies) {
          assert.equal((await requestAs(server, null, path, {
            method,
        ...
        for (const path of ["/catalog", "/inventory", "/cash-shifts", "/finance/summary", "/tabs", "/orders", "/kitchen/queue"]) {
        ...
        for (const path of ["/orders/missing/reprint", "/orders/missing/accept"]) {
          const status = (await requestAs(server, admin, path, { method: "POST", body: {} })).status;
          assert.notEqual(status, 401, path);
          assert.notEqual(status, 403, path);
        }
        assert.equal((await requestAs(server, admin, "/lgpd/anonymize", {
          method: "POST",
        ...
        const tabsBefore = Number((await pool.query("SELECT COUNT(*) FROM service_tabs")).rows[0].count);
        assert.equal((await requestAs(server, admin, "/tabs", {
          method: "POST",
          body: { kind: "tab", label: "csrf-cross-session" },
          csrf: otherAdmin.body.csrfToken
        })).status, 403);
        assert.equal(Number((await pool.query("SELECT COUNT(*) FROM service_tabs")).rows[0].count), tabsBefore);
        ...
        for (const path of ["/orders", "/tabs", "/cash-shifts", "/finance/summary", "/catalog", "/inventory"]) {
          assert.equal((await requestAs(server, operator, path)).status, 200, `operator ${path}`);
        }
        for (const path of ["/orders/x/accept", "/orders/x/reprint", "/demo/seed", "/lgpd/anonymize"]) {
          assert.equal((await requestAs(server, operator, path, {
            method: "POST",
            body: path === "/lgpd/anonymize" ? { searchTerm: "cliente" } : {}
          })).status, 403, `operator ${path}`);
        }
        ...
        for (const path of ["/cash-shifts", "/finance/summary", "/tabs", "/catalog", "/inventory"]) {
          assert.equal((await requestAs(server, kitchen, path)).status, 403, `kitchen ${path}`);
        }
        for (const path of ["/demo/seed", "/lgpd/anonymize"]) {
          assert.equal((await requestAs(server, kitchen, path, {
            method: "POST",
            body: path === "/lgpd/anonymize" ? { searchTerm: "cliente" } : {}
          })).status, 403, `kitchen ${path}`);
        }
    ...
    test("bootstrap concorrente cria um unico admin sem expor o segredo", async () => {
      ...
      try {
        const count = await pool.query("SELECT COUNT(*)::int count FROM users WHERE role = 'admin'");
    ...
    test("logout e expiracao absoluta revogam a sessao HTTP", async () => {
      ...
      try {
        ...
        assert.equal((await requestAs(server, loggedOut, "/auth/logout", {
          method: "POST"
        })).status, 204);
    ...
    test("SSE HTTP isola roles, revalida expiracao e nao entrega evento pendente", async () => {
      ...
      await pool.query(
        `INSERT INTO users (id, username, role, password_hash)
         VALUES ('kitchen-sse-test', 'kitchen-sse-test', 'kitchen', $1)
         ON CONFLICT (username) DO UPDATE SET password_hash = EXCLUDED.password_hash`,
        [passwordHash]
      );
      ...
      try {
        ...
        assert.equal((await requestAs(server, admin, "/orders/sse-auth-order/discount", {
          method: "PATCH",
    ...
    test("fechamento de comanda publica tab.closed v1 pós-commit para dois clientes", async () => {
      ...
      try {
        ...
        assert.equal((await pool.query(
          "SELECT status FROM orders WHERE id = 'p15-order'"
        ...
        assert.equal((await pool.query(
          "SELECT status FROM service_tabs WHERE id = 'p15-rollback-tab'"
        )).rows[0].status, "open");
      } finally {
    ...
    test("fila de impressão reconcilia ACK perdido, deduplica workers e protege dead-letter", async () => {
      ...
      try {
        ...
        assert.deepEqual((await pool.query(
          "SELECT status, attempts FROM print_jobs WHERE id = 'p16-concurrent'"
        )).rows[0], { status: "printed", attempts: 1 });
        ...
        await pool.query(
          `INSERT INTO users (id, username, role, password_hash)
           VALUES ('p16-operator', 'p16-operator', 'operator', $1)
        ...
        assert.equal((await requestAs(pair[0], operator, "/print-jobs/p16-dead/reprocess", {
          method: "POST",
          body: {}
        })).status, 403);
        ...
        assert.equal((await requestAs(pair[0], admin, "/print-jobs/p16-dead/reprocess", {
          method: "POST",
          body: {}
        })).status, 409);
    ...
    test("fingerprint idempotente cobre pedido, rodada, cancelamentos, legado e concorrencia", async () => {
      ...
      try {
        ...
        assert.equal((await requestAs(server, admin, `/tabs/${tab.body.id}/rounds`, {
          method: "POST",
          headers: { "Idempotency-Key": orderKey },
        ...
        const beforeEvents = Number((await pool.query("SELECT COUNT(*) FROM channel_events")).rows[0].count);
        ...
        assert.deepEqual(concurrentRequests.map((item) => item.status), [201, 201]);
        ...
        assert.equal(Number((await pool.query("SELECT COUNT(*) FROM channel_events")).rows[0].count), beforeEvents);
      } finally {
        await stopServer(server);
      }
    });
    ...
    test("fluxos manuais e integrados permanecem isolados por mapping e fallback legado", async () => {
      ...
      try {
        ...
        for (const id of ["p08-mapped", "p08-legacy"]) {
          ...
          assert.equal((await pool.query("SELECT status FROM orders WHERE id = $1", [id])).rows[0].status, "received");
        }
    ...
    test("outbox executa HTTP fora da transacao, possui lease e reconcilia sem reenvio", async () => {
      ...
      assert.equal((await pool.query(
        "SELECT status FROM channel_commands WHERE id = 'p10-concurrent'"
      )).rows[0].status, "completed");
      ...
      assert.equal((await pool.query(
        "SELECT status FROM channel_commands WHERE id = 'p10-finalize-failure'"
      ...
      assert.equal((await pool.query(
        "SELECT status FROM channel_commands WHERE id = 'p10-http-401'"
      )).rows[0].status, "ambiguous");
      await pool.query(
      ...
      await assert.rejects(
        db.transaction((client) => updateOwnedChannelCommand(
          "p10-ownership",
          "owner-errado",
      ...
      )).rows[0], { status: "ambiguous", attempts: 2 });
      await pool.query(
      ...
      assert.deepEqual((await pool.query(
        "SELECT status, attempts FROM channel_commands WHERE id = 'p11-watchdog'"
      )).rows[0], { status: "dead_letter", attempts: 3 });
    });
    ...
    test("integridade financeira, turno, ajustes idempotentes e adapter off", async () => {
      ...
      try {
        ...
        assert.equal((await pool.query("SELECT status FROM orders WHERE id = 'p03-sale'")).rows[0].status, "ready");
        assert.equal(Number((await pool.query(
          "SELECT COUNT(*) FROM finance_entries WHERE order_id = 'p03-sale'"
        )).rows[0].count), 0);
        ...
        assert.equal((await requestAs(server, admin, "/orders/p03-sale/status", {
          method: "PATCH",
          body: { status: "completed" }
        })).status, 200);
        const saleEntry = (await pool.query(
        ...
        assert.equal(Number((await pool.query("SELECT total FROM orders WHERE id = 'p03-sale'")).rows[0].total), 100);

        assert.equal((await requestAs(server, admin, "/orders/p03-sale/status", {
          method: "PATCH",
          body: { status: "cancelled", reason: "cliente" }
        })).status, 400);
        const manualCancelRace = await Promise.all([0, 1].map(() =>
        ...
        assert.deepEqual(manualCancelRace.map((response) => response.status), [200, 200]);
        assert.equal((await requestAs(server, admin, "/orders/p03-sale/status", {
          method: "PATCH",
          headers: { "Idempotency-Key": "p06-manual-cancel" },
        ...
        assert.deepEqual(ledger.rows.map((row) => Number(row.amount)), [100, -100]);
        assert.equal(ledger.rows.every((row) => row.shift_id === opened.body.id), true);
        assert.equal(ledger.rows.reduce((sum, row) => sum + Number(row.amount), 0), 0);
        assert.equal((await requestAs(server, admin, "/orders/p03-sale/discount", {
          method: "PATCH",
        ...
        const raceOrder = (await pool.query("SELECT total, status FROM orders WHERE id = 'p03-race'")).rows[0];
        const raceSale = (await pool.query(
        ...
        for (const [id, method] of [
          ...
          assert.equal((await requestAs(server, admin, `/orders/${id}/status`, {
            method: "PATCH",
            body: { status: "completed" }
          })).status, 200);
          assert.equal((await pool.query(
            ...
            [id]
          )).rows[0].shift_id, opened.body.id);
        }
        ...
        assert.equal((await requestAs(
          server,
          admin,
          `/cash-shifts/${adjustmentShift.body.id}/adjustments`,
          { method: "POST", body: { kind: "reinforcement", amount: 10, reason: "troco" } }
        )).status, 400);
        ...
        assert.equal(Number((await pool.query(
          "SELECT COUNT(*) FROM finance_entries WHERE id = $1",
        ...
        assert.deepEqual(concurrentAdjustments.map((response) => response.status), [200, 200]);
        assert.equal(concurrentAdjustments[0].body.entry.id, concurrentAdjustments[1].body.entry.id);
        assert.equal(Number((await pool.query(
        ...
        const currentShift = (await pool.query(
          "SELECT expected_amount FROM cash_shifts WHERE id = $1",
          [adjustmentShift.body.id]
        )).rows[0];
        assert.equal(Number(currentShift.expected_amount), 5);
        assert.equal((await requestAs(server, admin, `/cash-shifts/${adjustmentShift.body.id}/close`, {
          method: "POST",
        ...
        assert.equal((await requestAs(server, admin, `/cash-shifts/${nextShift.body.id}/adjustments`, {
          method: "POST",
          headers: { "Idempotency-Key": "p07-adjustment" },
          body: { kind: "reinforcement", amount: 10, reason: "troco" }
        })).status, 409);
        ...
        `);
        const commandsBefore = Number((await pool.query("SELECT COUNT(*) FROM channel_commands")).rows[0].count);
        ...
        assert.equal(Number((await pool.query("SELECT COUNT(*) FROM channel_commands")).rows[0].count), commandsBefore);
        assert.equal((await pool.query(
          "SELECT sync_status FROM channel_mappings WHERE id = 'p09-mapping'"
        )).rows[0].sync_status, "synchronized");
        assert.equal((await requestAs(
          server,
          admin,
          "/orders/p09-order/cancellation-reasons"
        )).status, 503);
    ...
    test("anonimizacao LGPD remove canarios, limpa spool e expõe pendencia sem sucesso falso", async () => {
      ...
      try {
        ...
        const fixtureQueries = [
          ["INSERT INTO service_tabs (id, kind, label, customer_name, status) VALUES ('lgpd-tab', 'tab', 'LGPD', $1, 'open')", [canary]],
          ["INSERT INTO cash_shifts (id, status, notes, opened_at) VALUES ('lgpd-shift', 'open', $1, NOW())", [canary]],
          [`INSERT INTO orders (
          ...
          [`INSERT INTO finance_entries (
             ...
             'counter', $1, $2::jsonb, NOW())`, [canary, metadataFixture]],
          [`INSERT INTO channel_mappings (
          ...
          [`INSERT INTO channel_commands (
             ...
             $1::jsonb, 'pending', NOW())`, [metadataFixture]],
          [`INSERT INTO order_tab_assignments (
             id, idempotency_key, order_id, tab_id, round_number, normalized_payload
          ...
          [`INSERT INTO tab_payments (
             ...
             'lgpd-payment-key', $1::jsonb)`, [metadataFixture]],
        ...
        assert.equal((await pool.query(
          "SELECT customer_name FROM orders WHERE id = 'lgpd-rollback-order'"
        )).rows[0].customer_name, rollbackCanary);
        assert.equal(Number((await pool.query(
        ...
        )).rows[0].status, "pending_external_cleanup");
      } finally {
        await stopServer(server);
        await stopPrintBridge(bridge);
      }
    ...
    test("troca de senha revoga todas as sessoes do usuario", async () => {
      ...
      try {
        ...
        const changed = await requestAs(server, first, "/auth/password", {
          method: "POST",
          body: { currentPassword: ADMIN_PASSWORD, newPassword: "postgres-test-admin-password-new" }
        });
        ...
        await requestAs(server, replacement, "/auth/password", {
          method: "POST",
          body: { currentPassword: "postgres-test-admin-password-new", newPassword: ADMIN_PASSWORD }
        });
      } finally {
        await stopServer(server);
      }
