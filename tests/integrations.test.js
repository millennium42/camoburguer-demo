...
import createDeliveryMuchAdapter, {
...
test("eventos iFood legados e atuais convergem para estados canônicos", () => {
  ...
  assert.equal(
    normalizeIFoodEventType({ fullCode: "ORDER_CONFIRMED", code: "CONFIRMED" }),
    "CONFIRMED",
  );
  assert.equal(
    normalizeIFoodEventType({ fullCode: "ORDER_CANCELLED", code: "CANCELLED" }),
    "CANCELLED",
  );
  assert.equal(
    normalizeIFoodEventType({ fullCode: "PREPARATION_ENDED", code: "SEPARATION_ENDED" }),
    "READY_TO_PICKUP",
  );
});
...
test("H-03 iFood: classificação reconcilia 'CANCELLED' durante 'accept' para localEffect 'cancel'", async () => {
  ...
  try {
    const res = await adapter.reconcileCommand({
      action: "accept",
      payload: { externalOrderId: "abc" },
    });
...
test("H-03 Delivery Much: classificação reconcilia 'cancelled' durante 'accept' para localEffect 'cancel'", async () => {
  const adapter = createDeliveryMuchAdapter(
    { deliveryMuch: { merchantId: "123", username: "a", password: "b", pollIntervalMs: 1000 } },
    {},
  );
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async (url) => {
    const data = String(url).includes("/login")
      ? { access_token: "token" }
      : { status: "cancelled" };
    return { ok: true, text: async () => JSON.stringify(data), json: async () => data };
  };
...
import { buildEntriesFromOrder, summarizeFinance } from "../packages/finance-core/index.js";
...
test("H-04 Finance Core: cancelamento com externalPayments reduz corretamente do summary", () => {
  ...
  const sales = buildEntriesFromOrder({
    order,
    previousStatus: "received",
    nextStatus: "completed",
    shiftId: "shift-1",
  });
  const cancellations = buildEntriesFromOrder({
    order,
